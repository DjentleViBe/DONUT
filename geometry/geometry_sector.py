...
def get_toroidal_coordinates_tangent(section_limits, points_3d):
    """
    Get tangents and coordinates of toridal sections
    """
    coords = []
    tangents = []
...
def is_inside_torus_axis(p0val, startcenter):
    """
    Returns True if P0 is on the inboard side (closer to torus axis than startcenter).
    All distances measured in XY plane only.
    """
    rmaj = np.sqrt(startcenter[0]**2 + startcenter[1]**2)        # major radius
    rmin = np.sqrt(p0val[0]**2 + p0val[1]**2)                          # P0's radial distance
    # inside = r < R
    # thickness = 0.25 if inside else 0.25
    tval = np.clip(abs(rmin - rmaj) / 0.15, 0.0, 1.0)
...
def guide_vane(point, vector, center, num_points):
    ...
    ctrl_pts = [p0val, p1val, p2val, p3val]
