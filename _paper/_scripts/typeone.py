...
def plot_typeone(points_2d, ctrl_2d,
                  points_3d, ctrl_3d,
                  moved_points_3d,
                  moved_ctrl_points,
    ...
    for i, x_section in enumerate(moved_points_3d[0]):
        ax3.plot(x_section, moved_points_3d[1][i], moved_points_3d[2][i], color=cfg.color[i], label = 'poloidal section ' + str(i))
        ax3.scatter(moved_ctrl_points[0][i], moved_ctrl_points[1][i], moved_ctrl_points[2][i], color=cfg.color[i], marker = '+',
                    depthshade=False)
        ax3.scatter(toroidal_coordinates[i][0],
                    toroidal_coordinates[i][1],
                     toroidal_coordinates[i][2], color='k', depthshade=False)
    ...
    ax3.text(0.7, 0.3, -0.1, r'$\mathbf{Q_4}$')
    plt.legend()
...
def geometry_construct(toroidal_sections, poloidal_sections, mode, init=False, plot=False, filename=None):
    ...
    if plot:
        # merge_stls(file_list, "./typeone.stl")
        plot_typeone([x_collections, y_collections], [ctrl_x_collections, ctrl_y_collections],
                  [x, y, z], [ctrl_x, ctrl_y, ctrl_z],
                  [x_moved_collections, y_moved_collections, z_moved_collections],
                  [x_moved_ctrl_collections, y_moved_ctrl_collections, z_moved_ctrl_collections],
                  toroidal_coordinates,
                  guide_vane_collections, "./typeone.stl",
                  filename=filename)
    return 0
...
geometry_construct(toroidal_sections, poloidal_sections,
                          1, plot=True, filename="./typeone_geometry")
